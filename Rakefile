...
def run_tests(platform, browser, version)
  system("platform=\"#{platform}\" browserName=\"#{browser}\" version=\"#{version}\" ruby sample_sauce.rb")
end

task :windows_8_1_chrome_43 do
  run_tests('Windows 8.1', 'chrome', '43')
end

task :windows_7_firefox_40 do
  run_tests('Windows 7', 'firefox','40')
end

task :os_x_10_9_chrome_45 do
  run_tests('OS X 10.9', 'chrome','45')
end

task :windows_xp_firefox_39 do
  run_tests('Windows XP', 'firefox','39')
end
